...
public class ClientController {
	...
	/**
	 * Parameter constructor for the client, assigns a view and a model to the
	 * controller.
	 * 
	 * @param cv The client view object, in control of the gui.
	 * @param cm The client model object, in control of client-server
	 *           communications.
	 */
	...
	/**
	 * Monitors the four buttons on the bottom right of the GUI: "Add", "Delete",
	 * "Modify", and "Clear".
	 */
	class ModifyListener implements ActionListener {
		...
		public boolean checkPostalCode(String postalCode) {
			...
			try {
				...
				if (check.length == 7 && numbers.matches("^[0-9]*$") && letters.matches("[a-zA-Z]+")
						&& check[3].equals(" ")) {
					test = true;
				} else
					test = false;
		...
		public boolean checkPhoneNumber(String phoneNumber) {
			...
			try {
				String numbers = check[0] + check[1] + check[2] + check[4] + check[5] + check[6] + check[8] + check[9]
						+ check[10] + check[11];
			...
			if (e.getSource() == clientGUI.eb1) {
				...
				if (!(checkPostalCode(postalCode))) {
					JOptionPane.showMessageDialog(null,
							"An invalid postal code was entered. Ensure your postal code follows"
									+ "the convention: A1A 1A1");
				} else if (!(checkPhoneNumber(phoneNumber))) {
					JOptionPane.showMessageDialog(null,
							"An invalid phone number was entered. Ensure your phone number follows"
									+ "the convention: 111-111-1111");
				} else if (firstName.length() > 20 && lastName.length() > 20) {
					JOptionPane.showMessageDialog(null,
							"The names entered were too long. Client names cannot exceed 20 letters");
				} else if (address.length() > 50) {
					JOptionPane.showMessageDialog(null,
							"The address entered was too long. Client address cannot exceed 50 characters");
			...
			if (e.getSource() == clientGUI.eb3) {
				try {
					...
					if (!(checkPostalCode(postalCode))) {
						...
						Client client = new Client(clientId, firstName, lastName, address, postalCode, phoneNumber,
								clientType);
						clientModel.modifyClient(client);
					}
				} catch (NumberFormatException e1) {
					JOptionPane.showMessageDialog(null,
							"The requested operation can not be performed. Please ensure that all relevant"
	...
	/**
	 * Listens to if the user selects any clients from the list on the bottom left
	 * of the GUI.
	 */
	...
	/**
	 * Tells the model object to connect to a specified IP address at the default
	 * port (9999).
	 * 
	...
	/**
	 * Tells the view object to display a dialog box polling for the user to enter
	...
	/**
	 * Tells the view object to display a confirmation dialog box for the addition
	 * of data.
	 * 
	...
	/**
	 * Tells the view object to display a confirmation dialog box for the deletion
	 * of data.
	 * 
	...
	/**
	 * Tells the view object to display a confirmation dialog box for the
	...
	/**
	 * Tells the view object to display a error dialog box for the failure of
	 * operation.
	 */
